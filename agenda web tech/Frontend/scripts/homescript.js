...
let input = ()=>{
    ...
    if(newTask!=''){
        $("form input").val('');
        let username = window.location.href.slice('http://127.0.0.1:8080/'.length)
        console.log(username)
        $.post(`http://127.0.0.1:8080/${username}/addTask`,
                {
                    username: username,
                    task: newTask
                },
...
let getTaskList = ()=>{
    let username = window.location.href.slice('http://127.0.0.1:8080/'.length)
    $.get(`http://127.0.0.1:8080/${username}/tasksList`, (data, status) => {
        console.log(data);
        ...
        taskList.forEach((task,index)=>{
            ...
            $(`#i${task._id}`).click(()=>{
                $.ajax({
                    url: `http://127.0.0.1:8080/delete/${task._id}`,
                    type: 'DELETE',
                    success: (result)=>{
                        if(result.delete){
                            getTaskList();
            ...
            $(`#d${task._id}`).click(()=>{
                window.location.href = `http://127.0.0.1:8080/details/${task._id}`
            })
        })
    })
}
